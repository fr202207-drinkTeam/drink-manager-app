...
const ItemDetail: FC = memo(() => {
  ...
  useEffect(() => {
    if (getAnItemResult.itemData) {
      console.log(getAnItemResult.itemData.images[0]);
      const itemImagePaths: string[] = getAnItemResult.itemData.images.map(
        (itemImage: {
          createdAt: Date;
          id: number;
          imagePath: string;
  ...
  const onClickDeleteItem: () => Promise<void> = async () => {
    const successDiscontinued: boolean = await DiscontinuedItemProcessing({
      itemId: itemId,
    });
